...
public class FoodNearbyAdapter extends RecyclerView.Adapter<FoodNearbyHolder> {
    ...
    public FoodNearbyAdapter(Context context, List<RestaurantModel> list) {
    ...
    @Override
    public FoodNearbyHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new FoodNearbyHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.food_list_item, parent, false));
    }
    ...
    @Override
    public void onBindViewHolder(final FoodNearbyHolder holder, int position) {
        ...
        if (resPropic == null || resPropic.equals("2")) {
            ...
            //set to gone for now (no API yet)
            ...
            holder.res_cam_btn.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    Intent takePropicIntent = new CameraHelper().startCameraIntent(context);
...
//        holder.res_loc_state.setText(resLoc + ", " + resState);
        ...
        String NumStar = String.format("%.1f", floatValue);
